...
from previous import module
#from common.fast_rcnn import FastRCNN
from visual_linguistic_bert import VisualLinguisticBertForPretraining
from previous import soft_cross_entropy
...
class ResNetVLBERTForPretraining(module):
    def __init__(self, model_name, embedding_size, hidden_size):

        super(ResNetVLBERTForPretraining, self).__init__()

        # self.image_feature_extractor = FastRCNN(config,
        #                                         average_pool=True,
        #                                         final_dim=config.NETWORK.IMAGE_FINAL_DIM,
        #                                         enable_cnn_reg_loss=False)
        self.object_linguistic_embeddings = nn.Embedding(1, embedding_size)
        self.object_mask_word_embedding = nn.Embedding(1, embedding_size)
        self.image_feature_bn_eval = True
        ...
        if os.path.isdir(model_name):
            weight_path = os.path.join(model_name, BERT_WEIGHTS_NAME)
        ...
        self.vlbert = VisualLinguisticBertForPretraining(
            ...
            with_rel_head=WITH_REL_LOSS,
            with_mlm_head=WITH_MLM_LOSS,
            with_mvrc_head=WITH_MVRC_LOSS,
        )
    ...
    def forward(self,
        ...
        mlm_loss = F.cross_entropy(mlm_logits.view((-1, mlm_logits.shape[-1])), mlm_labels.view(-1), ignore_index=-1)
        ...
        outputs.update({
            'relationship_logits': relationship_logits if self.WITH_REL_LOSS else None,
            'relationship_label': relationship_label if self.WITH_REL_LOSS else None,
            'mlm_logits': mlm_logits if self.WITH_MLM_LOSS else None,
            'mlm_label': mlm_labels if self.WITH_MLM_LOSS else None,
            'mvrc_logits': mvrc_logits if self.WITH_MVRC_LOSS else None,
            'mvrc_label': mvrc_labels if self.WITH_MVRC_LOSS else None,
